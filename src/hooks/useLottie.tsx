import React, { CSSProperties, useEffect, useRef, useState, ReactElement } from 'react'
import lottie, {
...
import { Listener, LottieOptions, LottieRefCurrentProps, PartialListener } from '#@types/lottie'
...
const useLottie = <T extends RendererType = 'svg'>(
  ...
  useEffect(() => {
    ...
    const [firstSegment, secondSegment] = initialSegment
    if (
      animationInstanceRef.current.currentRawFrame < firstSegment ||
      animationInstanceRef.current.currentRawFrame > secondSegment
    ) {
      animationInstanceRef.current.currentRawFrame = firstSegment
    }

    // Update the segment
    animationInstanceRef.current.setSegment(firstSegment, secondSegment)
  }, [initialSegment])
